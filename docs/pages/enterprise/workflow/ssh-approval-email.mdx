...
<Admonition type="warning" title="Be same with your email account">
...
set up a dedicated email account for the Teleport plugin.
...
the file's path. The plugin reads the content of `password_file` and uses the
literal value.
...
requested and identify the recipient to notify.

Here is an example of a `role_to_recipients` map:

```toml
[role_to_recipients]
"*" = ["security@example.com", "Executive Team <executive-team@example.com>"]
"dev" = ["Engineering <eng@example.com>"]
"dba" = "mallory@example.com"
```

In the `role_to_recipients` map, each key is the name of a Teleport role. Each
value configures the email recipients. The value can be a single string or an
array of strings. Each string must be an email address, either on its own
(`example@example.com`) or in angle brackets along with the recipient's name
...
Teleport email plugin to notify the same list of recipients for every Access
...
recipient list to a `role_to_recipients` mapping with only an entry for the
wildcard value `"*"`.
